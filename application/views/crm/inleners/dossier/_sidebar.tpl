			...
			<div class="card-body p-0">
				...
				<ul class="nav nav-sidebar" data-nav-type="accordion">
					...
					<li class="nav-item {if $inlener->complete != 1}order-6{/if}">
						<a {if ($inlener->cao_complete != NULL && $inlener->factuurgegevens_complete != NULL) || $inlener->complete == 1}href="crm/inleners/dossier/contactpersonen/{$inlener->inlener_id}"{/if} class="nav-link {if ($inlener->cao_complete == NULL || $inlener->factuurgegevens_complete == NULL) && $inlener->complete == 0}nav-link-disabled{/if} {if $active == 'contactpersonen'}active{/if}">
