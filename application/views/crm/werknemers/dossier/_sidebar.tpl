				...
				<ul class="nav nav-sidebar" data-nav-type="accordion">
                    ...
                    {if $user_type == 'werkgever'}
						...
						<li class="nav-item {if $werknemer->complete != 1}order-3{/if}">
							<a {if $werknemer->documenten_complete != NULL}href="crm/werknemers/dossier/dienstverband/{$werknemer->werknemer_id}"{/if} class="nav-link {if $werknemer->documenten_complete == NULL}nav-link-disabled{/if}{if $active == 'dienstverband'}active{/if}">
                                {* afwijkende icons voor nieuwe aanmelding *}
                                {if $werknemer->dienstverband_complete == NULL}
									<i class="icon-checkbox-unchecked2 mr-2"></i>
                                {else}
                                    {if $werknemer->complete == 0}
                                        {if $werknemer->dienstverband_complete == 0}
                        ...
                        {if $user_type == 'werkgever' }
							<li class="nav-item {if $werknemer->complete != 1 && $user_type == 'werkgever' }order-4{/if}">
								<a {if $werknemer->dienstverband_complete != NULL}href="crm/werknemers/dossier/verloning/{$werknemer->werknemer_id}"{/if} class="nav-link {if $werknemer->dienstverband_complete == NULL}nav-link-disabled{/if}{if $active == 'verloning'}active{/if}">
                                    ...
                                    {else}
                                        {if $werknemer->complete == 0}
                                            {if $werknemer->verloning_complete == 0}
												<i class="icon-pencil7 mr-2"></i>
                                            {/if}
                        ...
                        {if $werknemer->deelnemer_etregeling == 1}
							<li class="nav-item {if $werknemer->complete != 1 && $user_type == 'werkgever' }order-5{/if}">
								<a {if $werknemer->verloning_complete != NULL}href="crm/werknemers/dossier/etregeling/{$werknemer->werknemer_id}"{/if} class="nav-link {if $werknemer->verloning_complete == NULL}nav-link-disabled{/if}{if $active == 'etregeling'}active{/if}">
                                    {* afwijkende icons voor nieuwe aanmelding *}
                                    {if $werknemer->etregeling_complete == NULL}
										<i class="icon-checkbox-unchecked2 mr-2"></i>
                                    {else}
                                        {if $werknemer->complete == 0}
                                            {if $werknemer->etregeling_complete == 0}
												<i class="icon-pencil7 mr-2"></i>
                                            {/if}
                                            {if $werknemer->etregeling_complete == 1}
												<i class="icon-checkbox-checked mr-2"></i>
                                            {/if}
                                        {else}
                                            {* standaard icon *}
